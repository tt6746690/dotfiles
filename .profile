...
# meta submit
#
if [[ $(hostname) == submit-0  ]] && [[ $(uname) == Linux  ]]; then
    ...
    # >>> conda initialize >>>
    # !! Contents within this block are managed by 'conda init' !!
    __conda_setup="$('/data/home/wpq/miniconda3/bin/conda' 'shell.bash' 'hook' 2> /dev/null)"
    if [ $? -eq 0 ]; then
        eval "$__conda_setup"
    else
        if [ -f "/data/home/wpq/miniconda3/etc/profile.d/conda.sh" ]; then
            . "/data/home/wpq/miniconda3/etc/profile.d/conda.sh"
        else
            export PATH="/data/home/wpq/miniconda3/bin:$PATH"
        fi
    fi
    unset __conda_setup
    # <<< conda initialize <<<
